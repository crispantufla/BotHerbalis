...
const RULE_BASE = [
    ...
    { id: 'medios_pago', keywords: ['qr', 'mercadopago', 'mercadolibre', 'transferencia', 'debito', 'credito', 'cbu', 'alias', 'tarjeta'], text: 'MEDIOS DE PAGO: Ofrecemos 3 opciones cuando el cliente está por elegir cómo paga: (1) *Tarjeta de crédito o débito* — por Mercado Pago, link inmediato, cubre crédito, débito y saldo MP; (2) *Transferencia bancaria* — alias *ERRONEA.HABLAME.LUZ* a nombre de *Bio Origen SAS*, monto total; (3) *Contra reembolso* — anticipo de *$10.000* por transferencia al mismo alias (cubre el envío) + saldo en efectivo al cartero al recibir. La opción 3 aplica a TODOS los planes y a TODOS los clientes. NUNCA menciones cuotas (ni "tarjeta en cuotas", ni "podés pagar en cuotas") — quien quiera dividir el pago verá lo que su tarjeta permita al abrir el link de MP, pero el bot NO promete ni menciona cuotas. NO menciones "efectivo en Pago Fácil/Rapipago". NO inventes CBUs ni titulares distintos al oficial.' },
...
MEDIOS DE PAGO:
- Ofrecemos 3 opciones espontáneamente: (1) Tarjeta de crédito o débito por Mercado Pago, (2) Transferencia bancaria al alias ERRONEA.HABLAME.LUZ a nombre de Bio Origen SAS, (3) Contra reembolso con anticipo de $10.000 al mismo alias + saldo en efectivo al cartero.
    ...
    async chat(userText: string, context: APIContext): Promise<AIParsedResponse> {
        ...
        if (context.knowledge && context.knowledge.flow) {
            ...
            if (['waiting_weight', 'waiting_preference'].includes(step)) {
                ...
                knowledgeContext += `- POLÍTICA DE PAGO: 3 opciones espontáneas — (1) link de Mercado Pago para tarjeta de crédito o débito, (2) transferencia bancaria al alias ERRONEA.HABLAME.LUZ (Bio Origen SAS), (3) contra reembolso con anticipo de $10.000 al mismo alias + saldo en efectivo al cartero. Aplica a TODOS los planes. NUNCA menciones cuotas.\n`;
