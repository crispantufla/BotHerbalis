...
const RULE_BASE = [
    ...
    { id: 'pago', keywords: ['pago', 'se paga', 'como abono', 'cuando abono', 'como se abona', 'cuando pago', 'efectivo'], text: 'PREGUNTAS SOBRE PAGO (modelo nuevo may-2026): Si el usuario pregunta "¿se paga cuando me lo traen?", "¿cómo se paga?" o sobre el método de pago: ACLARALE que tenemos 2 opciones de envío: 1) *Retiro en sucursal* — contrarrembolso, pagás el total en efectivo cuando lo retirás en una sucursal de Correo Argentino (sin anticipo); 2) *Envío a domicilio* — se abona previamente con *tarjeta de crédito* (link de pago) o *transferencia bancaria* al alias HERBALIS.TIENDA. Ambos son gratis. De cara al cliente el medio online se llama "Tarjeta de crédito" (NUNCA "Mercado Pago", débito, Pago Fácil ni Rapipago). NUNCA mencionés anticipo de $10.000 ni cuotas. Luego retomá la conversación.' },
    ...
    { id: 'medios_pago', keywords: ['qr', 'mercadopago', 'mercadolibre', 'transferencia', 'debito', 'credito', 'cbu', 'alias', 'tarjeta'], text: 'MEDIOS DE PAGO (modelo nuevo may-2026): Hay 2 tipos de envío y según cuál elija, los medios son distintos. (A) *Retiro en sucursal* → contrarreembolso: el cliente paga el TOTAL en efectivo cuando va a la sucursal de Correo Argentino. Sin anticipo, sin transferencia previa. (B) *Envío a domicilio* → prepago, antes de despachar. Las opciones de prepago son: (1) *Tarjeta de crédito* — link de pago inmediato y 100% protegido; (2) *Transferencia bancaria* al alias *HERBALIS.TIENDA* a nombre de *BIO ORIGEN S.A.S.* por el monto total (y si el cliente lo pide, también puede transferir y retirar en sucursal). Ambos envíos son GRATIS. De cara al cliente el medio online se llama SIEMPRE "Tarjeta de crédito" — NUNCA "Mercado Pago", débito, app, Pago Fácil ni Rapipago (esas opciones ya no se ofrecen, decisión jun-2026). NUNCA menciones "anticipo de $10.000" — esa modalidad fue eliminada en mayo 2026. NUNCA menciones cuotas. NO inventes CBUs ni titulares distintos al oficial.' },
...
- Envío GRATIS por Correo Argentino (7 a 10 días hábiles). Dos opciones: retiro en sucursal (pago al retirar) o envío a domicilio (prepago con tarjeta de crédito o transferencia).
    ...
    async chat(userText: string, context: APIContext): Promise<AIParsedResponse> {
        ...
        if (context.knowledge && context.knowledge.flow) {
            ...
            if (['waiting_weight', 'waiting_preference'].includes(step)) {
                ...
                knowledgeContext += `- ENVÍO Y PAGO: Envío gratis por Correo Argentino (7 a 10 días hábiles). 2 opciones: retiro en sucursal (paga al retirar) o envío a domicilio (prepago con tarjeta de crédito o transferencia). NUNCA menciones cuotas ni anticipo.\n`;
