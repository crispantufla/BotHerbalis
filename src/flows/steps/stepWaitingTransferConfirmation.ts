...
export async function handleWaitingTransferConfirmation(
    ...
    const aiRes = await aiService.chat(text, {
        ...
        goal: `El cliente ${isCodAnticipo ? 'eligió pagar contra reembolso y debe enviar el *anticipo de $10.000* por transferencia' : 'eligió pagar por transferencia bancaria'} al alias *HERBALIS.TIENDA* a nombre de *Bio Origen SAS*. Estás esperando que confirme que ${isCodAnticipo ? 'envió el anticipo' : 'realizó la transferencia'}.\n\nREGLAS:\n1. Si pregunta el alias, titular o monto de nuevo, recordáselo: alias *HERBALIS.TIENDA*, a nombre de *Bio Origen SAS*, monto ${isCodAnticipo ? '*$10.000* (anticipo — el resto en efectivo al cartero)' : `$${currentState.totalPrice || '0'}`}.\n2. Si dice que ya transfirió ("listo", "hecho", "ya hice la transferencia"), confirmá que verificás el pago.\n3. Si quiere cambiar a otro método, ofrecele las otras opciones.\n4. Si tiene dudas sobre cómo transferir, explicale que puede hacerlo desde su home banking o app del banco usando el alias.\n\nNUNCA inventes datos bancarios más allá del alias y titular oficiales. Hablá siempre en primera persona como Elena, con calidez.`,
