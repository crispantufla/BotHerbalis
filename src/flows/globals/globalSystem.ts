...
export async function handleSystemGlobals(
    ...
    if (currentState.pendingCancelConfirm && currentState.step !== 'completed') {
        ...
        if (confirmYes) {
            ...
            const byeMsg = 'Entendido, lamentamos no poder ayudarte en esta ocasión 😔 ¡Si en algún momento nos necesitas, aquí estamos!';
            ...
            // Ambiguous — ask again
            const askMsg = 'Perdona, ¿me confirmas que quieres cancelarlo? Responde *sí* o *no* 😊';
    ...
    if (((EXPLICIT_CANCEL_REGEX.test(normalizedText) && !NO_CANCEL_PHRASE.test(normalizedText))
        ...
        const msg = '¿Seguro que no quieres seguir? Antes de decidirlo, puedo resolverte cualquier duda 😊\n\nResponde *sí* para cancelar o *no* para seguir.';
    ...
    // 2.6 PAGO FANTASMA / POSIBLE ESTAFA
    // El cliente dice que YA pagó o que mandó un justificante. Aquí eso NUNCA
    // debería pasar: todo se cobra contra reembolso y el bot no da ninguna
    // cuenta, ningún enlace ni ningún Bizum. Si alguien dice que ya pagó, o se
    // ha confundido de proveedor, o un tercero se ha hecho pasar por nosotros y
    // le ha sacado el dinero. El bot NO sigue vendiendo: corta, le pide que no
    // haga más pagos y avisa URGENTE a una persona.
    const CLAIMS_PAID_REGEX = /\b(ya (lo )?pague|ya (lo )?abone|ya transferi|ya hice (la )?transferencia|hice (la )?transferencia|ya esta pag[oa]|ya lo ingrese|ya hice el bizum|te pase el comprobante|te mande el justificante|te mande el comprobante|ya hice el pago|ya pague todo)\b/;
    const PAYMENT_STEPS = ['waiting_admin_validation', 'completed'];
    if (CLAIMS_PAID_REGEX.test(normalizedText) && !PAYMENT_STEPS.includes(currentState.step)) {
        logger.warn(`[PAGO-CONFUSO] User ${userId} dice que ya pagó pero aquí NADIE paga por adelantado (step=${currentState.step}) — posible estafa. Pauso + alerto.`);
        const msg = 'Espera un momento que reviso bien tu caso 🙏 Déjame comprobarlo con el equipo y enseguida te escribo. Por si acaso, NO hagas ningún otro pago hasta que te confirme, ¿vale? 😊';
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
    ...
    const GEO_REGEX = /\b(argentina|mexico|méxico|chile|colombia|peru|perú|uruguay|bolivia|paraguay|ecuador|venezuela|brasil|panama|panamá|costa rica|eeuu|ee\.?\s?uu\.?|estados unidos|francia|portugal|alemania|reino unido|inglaterra|italia|marruecos|andorra|fuera del pais|fuera de espana|otro pais|no estoy en espana|vivo en el extranjero|desde fuera|no soy de espana)\b/i;
    // Negaciones explícitas de pertenencia — éstas SÍ son extranjero real, NO una
    // aclaración (evita que "no estoy en españa" cuente como señal española).
    const GEO_NEG_ES = /\b(no estoy en espana|fuera de espana|no soy de espana|no es espana)\b/i;
    // Señal de identidad ESPAÑOLA: nombra España, su comunidad/provincia, o una
    // población ambigua que contiene un nombre de país (Nueva Guinea en Ávila,
    // El Brasil en Ourense…). NO se enumeran poblaciones sueltas (sería
    // whack-a-mole): basta con que el cliente nombre "españa" o su provincia en
    // positivo. Cubre tanto la aclaración tras un rechazo ("está en España")
    // como la identificación temprana ("soy de Cádiz"), que debe inmunizarlo
    // contra un "estoy en Francia" posterior.
    const GEO_ES_IDENTITY = /\b(espana|peninsula|canarias|baleares|ceuta|melilla|andalucia|cataluna|catalunya|galicia|asturias|cantabria|euskadi|pais vasco|navarra|la rioja|aragon|castilla|extremadura|valencia|comunidad valenciana|murcia|madrid|provincia)\b/i;
    // Marco de "español de viaje": menciona el extranjero PERO con regreso o
    // compra a futuro. No es un cliente de fuera — ante la duda pausamos.
    ...
    const clarifiesSpain = GEO_ES_IDENTITY.test(normalizedText) && !GEO_NEG_ES.test(normalizedText);

    // Una vez que el cliente se identifica como residente en España queda
    // inmunizado: las keywords de extranjero posteriores (de viaje, compra al
    // volver) ya no lo rechazan.
    if (clarifiesSpain) currentState.spainConfirmed = true;

    if (GEO_REGEX.test(normalizedText) && !clarifiesSpain && !currentState.spainConfirmed && !currentState.geoRejected && !isCollectingAddress) {
        // Cliente de aquí que está de viaje (menciona el extranjero + regreso o
        // compra futura): pausamos y derivamos en vez de rechazar y perder la venta.
        if (TRAVEL_FRAME.test(normalizedText)) {
            logger.info(`[GEO REJECT] User ${userId} menciona el extranjero con marco de viaje/compra futura → pauso en vez de rechazar: "${text}"`);
            await _pauseAndAlert(userId, currentState, dependencies, text, '✈️ Menciona estar fuera de España pero con marco de viaje / compra al volver. Posible cliente de aquí de viaje — revisar y continuar la venta a mano.');
            saveState(userId);
            return { matched: true };
        }
        logger.info(`[GEO REJECT] User ${userId} is outside Spain: "${text}"`);
        currentState.geoRejected = true;
        const msg = 'Por desgracia solo hacemos envíos dentro de España 😔 Si en algún momento lo necesitas para alguien de aquí, ¡te ayudamos encantados!';
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
    ...
    if (currentState.geoRejected || currentState.step === 'rejected_geo') {
        // Recuperación de falso positivo: si el cliente aclara que SÍ está en
        // España (ej: "es que soy de Cádiz" / "estoy en España"), levantamos el
        // rechazo y lo derivamos a un humano en vez de seguir bloqueando.
        if (clarifiesSpain || currentState.spainConfirmed) {
            logger.info(`[GEO REJECT] User ${userId} aclara que está en España ("${text}") → levanto geoRejected y derivo a admin.`);
            currentState.geoRejected = false;
            _setStep(currentState, 'greeting');
            await _pauseAndAlert(userId, currentState, dependencies, text, '📍 Cliente geo-rechazado que aclara estar en España (posible falso positivo). Revisar y continuar la venta a mano.');
            saveState(userId);
            return { matched: true };
        }
        logger.info(`[GEO REJECT] User ${userId} already geo-rejected, blocking.`);
        const msg = 'Como te comentaba, por desgracia solo hacemos envíos dentro de España 😔';
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
        return { matched: true };
    }
