 ...
 *  - caso real de CP pegado a la poblacion: "Getafe Libertad 28901" → el CP 1716 no se detectaba (lo tomaba
...
// ════════════════════════════════════════════════════════════════════════════
// 1. CP pegado a la localidad — "Getafe Libertad 28901" → CP 1716 (caso real de CP pegado a la poblacion)
// ════════════════════════════════════════════════════════════════════════════
describe('Retiro — CP embebido en "localidad + número"', () => {
    test('"Getafe Libertad 28901" arma la orden con cp=28901 aunque el parser no lo extraiga', async () => {
        const sent = [];
        const deps = {
            ...
            mockAiService: { parseAddress: async () => ({ ciudad: 'Getafe' }) },
        };
        const state = baseState({ partialAddress: { calle: 'A oficina de Correos', nombre: 'Maria Jose Robledo', ciudad: 'Getafe' } });
        const text = 'Getafe Libertad 28901';
        const res = await handleWaitingData('cp1@c.us', text, norm(text), state, { flow: {} }, deps);
        expect(res.matched).toBe(true);
        expect(state.pendingOrder).toBeTruthy();
        expect(state.pendingOrder.cp).toBe('28901');
...
describe('Waiting data — el cliente pide esperar', () => {
    test('"Ok, un momento, por favor" → afloja y NO re-pide datos', async () => {
        ...
        const state = baseState({ partialAddress: { calle: 'A oficina de Correos', ciudad: 'Getafe' } }); // falta nombre
    ...
    test('"Sisi, un segundo, ya me desocupo, y estoy con ud" → afloja, no apura', async () => {
        ...
        const state = baseState({ partialAddress: { calle: 'A oficina de Correos', ciudad: 'Getafe' } });
    ...
    test('tras pedir esperar, "Ok, gracias, muy amable" NO dispara re-pedido de datos', async () => {
        const sent = [];
        const deps = { sendMessageWithDelay: async (_id, m) => { sent.push(m); }, saveState: jest.fn(), aiService: { parseAddress: async () => ({}) } };
        const state = baseState({ awaitingResume: true, partialAddress: { calle: 'A oficina de Correos', ciudad: 'Getafe' } });
    ...
    test('un mensaje con datos reales SÍ retoma (limpia awaitingResume y arma orden)', async () => {
        ...
        const state = baseState({ awaitingResume: true, partialAddress: { calle: 'A oficina de Correos', ciudad: 'Getafe', cp: '28901' } });
