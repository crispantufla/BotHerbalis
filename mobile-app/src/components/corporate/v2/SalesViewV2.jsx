    ...
    return (
        <div className="h-full flex flex-col animate-fade-in relative z-10 w-full space-y-2 sm:space-y-6">
            ...
            <div className="bg-white/4 dark:bg-slate-800/40 backdrop-blur-xl rounded-[1.5rem] sm:rounded-[2rem] border border-white/6 dark:border-slate-700/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-2.5 sm:p-5 lg:p-6">
                <div className="flex flex-row justify-between items-center gap-2 mb-2 sm:mb-5">
                    <div className="min-w-0">
                        <h1 className="text-base sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-purple-600 tracking-tight truncate">
                            Logística y Pedidos
                        </h1>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                        <button onClick={() => refetch()} className="p-2 sm:p-3 bg-white border border-slate-200 rounded-xl text-slate-600 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 transition-all shadow-sm active:scale-95 group flex items-center justify-center">
                            <span className={`group-hover:rotate-180 transition-transform duration-500 flex ${isFetching ? 'animate-spin text-indigo-500' : ''}`}><Refresh className="w-4 h-4" /></span>
                        </button>
                        <button onClick={handleExportCSV} disabled={orders.length === 0} className="px-2.5 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl text-xs sm:text-sm font-bold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:scale-105 transition-all flex items-center justify-center gap-1 sm:gap-2 disabled:opacity-50 disabled:scale-100">
            ...
            <div className="flex-1 bg-white/6 dark:bg-slate-800/60 backdrop-blur-xl rounded-[1.25rem] sm:rounded-[2rem] border border-white/8 dark:border-slate-700/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col relative text-sm">
                ...
                <div className="px-2 sm:px-8 py-3 sm:py-4 border-t border-white/6 dark:border-slate-700/60 flex flex-wrap sm:flex-nowrap justify-between justify-center sm:justify-between items-center bg-white/3 dark:bg-slate-800/30 backdrop-blur-md gap-y-3 gap-x-2">
                    <span className="text-[10px] sm:text-sm font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap order-1">
                        <span className="text-indigo-600">{page}</span>/{pagination.totalPages}
                        <span className="hidden sm:inline"> Página</span>
                    </span>
                    <div className="flex gap-2 w-full sm:w-auto order-3 sm:order-2 justify-center">
                        <button
                    ...
                    <span className="text-[10px] sm:text-sm font-bold text-indigo-500 bg-indigo-50 px-2 sm:px-3 py-1 rounded-full border border-indigo-100 whitespace-nowrap order-2 sm:order-3">{pagination.total} <span className="hidden sm:inline">Totales en BD</span><span className="sm:hidden">totales</span></span>
                </div>
            </div>
