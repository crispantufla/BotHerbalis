...
const StatsPanel = ({ stats, loadingStats, alertsCount = 0 }) => {
    ...
    const StatCard = ({ title, value, subtext, icon: Icon, colorClass, gradientClass }) => (
        <div className="relative overflow-hidden bg-white/6 dark:bg-slate-800/60 backdrop-blur-xl p-2.5 sm:p-4 lg:p-5 2xl:p-7 rounded-[1rem] sm:rounded-[1.25rem] border border-white/8 dark:border-slate-700/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 group hover:-translate-y-1">
            ...
            <div className="relative z-10">
                <h3 className="text-lg sm:text-2xl 2xl:text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight leading-tight">{value}</h3>
                <p className="text-[9.5px] sm:text-[13px] 2xl:text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium leading-tight">{subtext}</p>
            </div>
        </div>
    );
