        ...
        <div className="flex-1 w-full min-h-0 flex flex-col md:flex-row animate-fade-in relative overflow-hidden bg-slate-50 dark:bg-slate-900 rounded-[2rem] border border-white/6 dark:border-slate-700/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <div className={`w-full md:w-72 lg:w-[300px] xl:w-[340px] 2xl:w-[400px] flex-shrink-0 border-r border-slate-200 dark:border-slate-800 flex-col bg-white dark:bg-slate-800 z-10 ${selectedChat ? 'hidden md:flex' : 'flex'} min-h-0`}>
                ...
                <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800">
                    <div className="relative group">
                        <input
                            ...
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-12 pr-4 py-2 sm:py-3 text-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-300 transition-all text-slate-800 dark:text-slate-100 shadow-inner font-medium placeholder:text-slate-400"
                        />
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                    {isLoadingChats && chats.length === 0 ? (
                        ...
                        <div key={chat.id} onClick={() => setSelectedChat(chat)} className={`p-3 sm:p-4 mb-1 sm:mb-2 rounded-2xl flex cursor-pointer transition-all duration-300 ${selectedChat?.id === chat.id ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/30 transform scale-[1.02]' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>
            ...
            <div className={`flex-1 flex-col min-w-0 bg-transparent relative z-0 ${selectedChat ? 'flex' : 'hidden md:flex'} min-h-0`}>
                {selectedChat ? (
                    <>
                        {/* Header */}
                        <div className="flex-shrink-0 min-h-[4rem] sm:min-h-[5rem] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-3 sm:px-8 bg-white dark:bg-slate-800 shadow-[0_2px_10px_rgba(0,0,0,0.02)] z-20 overflow-hidden py-1.5 sm:py-2 gap-2">
