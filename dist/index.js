...
redisClient.on('error', (err) => logger.error(`[REDIS REDLOCK] Error de conexión: ${err?.message || String(err)}`));
...
const userState = new Proxy({}, {
    get: (target, prop) => {
        if (prop === 'constructor' || typeof prop === 'symbol' || prop === 'then' || prop === 'toJSON')
    ...
    getOwnPropertyDescriptor: (target, prop) => {
        if (userCache.has(prop))
            return { enumerable: true, configurable: true, value: userCache.get(prop) };
        return undefined;
    }
});
...
const lastPausedUserAlerts = new Map();
...
restorePausedUsersFromDB({ pausedUsers }).catch((e) => logger.error('[BOOT] Failed to restore paused users:', e.message));
const pendingMessages = new Map(); // Debounce: userId -> { messages: [{text, timestamp}], timer }
...
const _pendingSaveUsers = new Set();
let _isSaving = false;
async function _persistState() {
    ...
    try {
        ...
        const userPromises = usersToSave.map(([phone, data]) => {
            ...
            const lastSeenDate = data?.lastActivityAt ? new Date(data.lastActivityAt) : new Date();
    ...
    try {
        ...
        dbConfig.forEach((c) => {
            try {
                ...
                logger.warn(`[LOAD] Failed to parse config key "${c.key}":`, e.message);
            }
        });
        ...
        dbUsers.forEach((u) => {
            if (u.profileData) {
                try {
                    ...
                    logger.warn(`[LOAD] Failed to parse profile for ${u.phone}:`, e.message);
                }
            }
        });
        ...
        if (fs.existsSync(STATE_FILE)) {
            try {
                ...
                if (data.pausedUsers && Array.isArray(data.pausedUsers)) {
                    data.pausedUsers.forEach((userId) => pausedUsers.add(userId));
                }
                if (data.chatResets)
                    Object.assign(chatResets, data.chatResets);
                if (data.lastAlertUser)
                    lastAlertUser = data.lastAlertUser;
            }
            catch (e) {
...
function normalizeProductName(rawProduct, rawPlan, price) {
    const lower = (rawProduct || '').toLowerCase();
    let baseType = '';
    if (lower.includes('capsul') || lower.includes('cápsul'))
        baseType = 'Cápsulas';
    else if (lower.includes('gota'))
        baseType = 'Gotas';
    else if (lower.includes('semilla'))
        baseType = 'Semillas';
    if (!baseType)
        return rawProduct || 'Desconocido';
    ...
    if (!duration || duration % 60 !== 0) {
        if (baseType === 'Cápsulas')
            duration = price >= 66900 ? 120 : 60;
        else if (baseType === 'Gotas')
            duration = price >= 68900 ? 120 : 60;
        else if (baseType === 'Semillas')
            duration = price >= 49900 ? 120 : 60;
    }
...
function saveOrderToLocal(order) {
    ...
    (async () => {
        ...
        try {
            ...
            if (sharedState.io)
                sharedState.io.emit('new_order', legacyFormatOrder);
        }
        catch (e) {
...
async function cancelLatestOrder(userId) {
    ...
    try {
        ...
        if (sharedState.io)
            sharedState.io.emit('order_update', legacyFormatUpdate);
...
const sendMessageWithDelay = async (chatId, content, startTime = Date.now()) => {
    ...
    const isAdminChat = alertNums.some((n) => chatId.startsWith(n));
...
async function handleAdminCommand(targetChatId, commandText, isApi = false, alertSelector = null) {
...
client.on('ready', () => {
    ...
    if (sharedState.io)
        sharedState.io.emit('ready', { info: client.info });
...
client.on('auth_failure', (msg) => {
    ...
    if (sharedState.io)
        sharedState.io.emit('status_change', { status: 'auth_failure' });
...
client.on('disconnected', (reason) => {
    ...
    sharedState.qrCodeData = null;
    if (sharedState.io)
        sharedState.io.emit('status_change', { status: 'disconnected' });
    ...
    try {
        ...
        if (isAdmin) {
            ...
            if (msgText.toLowerCase().startsWith('!saltear ')) {
                ...
                if (!userState[targetChatId])
                    userState[targetChatId] = { step: 'greeting', partialAddress: {} };
        ...
        if (pausedUsers.has(userId) || (msg.from !== userId && pausedUsers.has(msg.from))) {
            ...
            if (now - lastAlert > 30 * 60 * 1000) {
                ...
                try {
                    await notifyAdmin('💬 Cliente en pausa te escribió', userId, `El cliente envió un mensaje: "${msgText.substring(0, 100)}..."\n\nEl bot sigue pausado automático.`);
                }
                catch (e) {
...
// Graceful Shutdown
const _shutdown = async (signal, exitCode = 0) => {
    ...
    try {
        ...
        logger.error('[SHUTDOWN] Error flushing state:', e.message);
    }
    try {
        ...
        logger.error('[SHUTDOWN] Error destroying client:', e.message);
    }
    try {
        await shutdownQueue();
    }
    catch (e) {
        logger.error('[SHUTDOWN] Error closing Redis/Queue:', e.message);
    }
    try {
        ...
        logger.error('[SHUTDOWN] Error cleaning pause service:', e.message);
    }
    try {
        if (sharedState.io) {
    ...
    try {
        ...
        logger.error('[SHUTDOWN] Error closing HTTP server:', e.message);
    }
    try {
        ...
        logger.error('[SHUTDOWN] Error closing DB:', e.message);
    }
