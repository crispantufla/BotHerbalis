...
describe('V3 Script — Price Centralization', () => {
    ...
    test('Should replace {{PRICE_...}} placeholders with real values (Semillas via preference)', async () => {
        ...
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.not.stringMatching(/{{PRICE_/));
        // Expect specific price for Semillas 60 days
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.stringMatching(/36\.900/));
    });
    test('Should show updated Cápsulas 60 price ($46.900)', async () => {
        ...
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.stringMatching(/46\.900/));
    });
});
...
describe('V3 Script — FAQ Keywords', () => {
    ...
    test.skip('FAQ: trust concern with tarjeta mention triggers payment FAQ', async () => {
        ...
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.stringContaining("pago al recibir"));
    });
    ...
    test.skip('FAQ: "contraindicaciones" should respond about pregnancy', async () => {
        ...
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.stringContaining("embarazo"));
    });
    ...
    test.skip('FAQ: "costo de envio" should respond about free shipping', async () => {
        ...
        expect(mockSendMessage).toHaveBeenCalledWith(userId, expect.stringContaining("gratuito"));
    });
});
