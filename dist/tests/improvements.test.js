...
jest.mock('fs', () => {
    ...
    return {
        ...
        existsSync: jest.fn((p) => {
            if (typeof p === 'string' && (p.endsWith('prices.json') || p.endsWith('knowledge_v3.json')))
                return true;
...
describe('Strategic Improvements', () => {
    ...
    beforeEach(() => {
        ...
        aiService.parseAddress.mockImplementation(async (text) => {
            if (text.includes("Calle Falsa"))
                return { calle: "Calle Falsa 123" };
    ...
    test('1. Consultative Tagging: Should mark sales as consultative', async () => {
        ...
        expect(mockDependencies.sendMessageWithDelay).toHaveBeenCalledWith(userId, "Te recomiendo gotas.");
    });
    ...
    test('3. Smart Address Accumulation: Should acknowledge partial progress', async () => {
        ...
        expect(mockDependencies.sendMessageWithDelay).toHaveBeenCalledWith(userId, expect.stringMatching(/Ya agendé|pendiente|Ya casi estamos/i));
    ...
    test('4. Postponement Handling: Should NOT force closing question if user is busy', async () => {
        ...
        expect(mockDependencies.sendMessageWithDelay).toHaveBeenCalledWith(userId, "Dale, tranqui. Avisame cuando puedas! 😊");
    ...
    test('5. Full Price List: Should show all prices if explicitly asked', async () => {
        ...
        expect(mockDependencies.sendMessageWithDelay).toHaveBeenCalledWith(userId, expect.stringContaining("Acá tenés todo"));
    });
});
