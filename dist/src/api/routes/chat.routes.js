...
module.exports = (client, sharedState) => {
            ...
            for (const item of preProcessedChats) {
                ...
                if (!resolvedChatsMap.has(resolvedId)) {
                    ...
                    if (isExistingLid && !isNewLid)
                        ...
                        finalName = existing.resolvedName;
                    // If one is purely numeric and the other has format, prefer formatted
                    else if (finalName === existing.resolvedId.replace(/\D/g, '') && isNaN(resolvedName.replace(/\D/g, '')))
                        finalName = resolvedName;
                    ...
                    if (m.hasMedia && !body) {
                        if (m.type === 'audio' || m.type === 'ptt') {
                            ...
                            if (fs.existsSync(audioPath)) {
                                ...
                                try {
                                    const media = await withTimeout(m.downloadMedia(), 10000, "Audio download timeout");
                                    if (media) {
            ...
            if (!pausedUsers.has(chatId)) {
                ...
                if (sharedState.io) {
                    ...
                    if (originalChatId !== chatId)
                        sharedState.io.emit('bot_status_change', { chatId: originalChatId, paused: true });
                }
            }
            if (sharedState.logAndEmit)
                sharedState.logAndEmit(chatId, 'admin', message, 'dashboard_reply', sentMsg.id._serialized);
    ...
    router.post('/reset-chat', authMiddleware, async (req, res) => {
        try {
            ...
            if (sharedState.logAndEmit)
                sharedState.logAndEmit(chatId, 'system', 'Memoria de chat reiniciada', 'new');
