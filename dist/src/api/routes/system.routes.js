...
module.exports = (client, sharedState) => {
    ...
    router.get('/scan', authMiddleware, (req, res) => {
        ...
        if (!qrData)
            return res.send('<h1>No QR Code active</h1><p>Bot is either connected or initializing. Check logs.</p>');
    ...
    router.delete('/alerts/:userPhone', authMiddleware, (req, res) => {
        ...
        if (index !== -1) {
            ...
            if (io)
                io.emit('alerts_updated', sessionAlerts);
    ...
    router.get('/stats', authMiddleware, async (req, res) => {
        try {
            ...
            const activeConversations = Object.values(userState || {}).filter(s => s && s.step && s.step !== 'completed' && s.step !== 'greeting').length;
            res.json({
                todayRevenue,
                todayOrders,
                totalOrders,
    ...
    router.post('/global-pause', authMiddleware, (req, res) => {
        try {
            ...
            if (io)
                io.emit('global_pause_changed', { globalPause: config.globalPause });
        ...
        try {
            if (client && client.info && sharedState.isConnected) {
                ...
                (async () => {
                    ...
                    catch (e) { /* Chrome may not have been started yet */ }
    ...
    router.post('/pairing-code', authMiddleware, validate(pairingCodeSchema), async (req, res) => {
        try {
            ...
            if (!phoneNumber)
                return res.status(400).json({ error: 'Falta el campo "phoneNumber"' });
    ...
    router.post('/script/switch', authMiddleware, validate(scriptSwitchSchema), (req, res) => {
        try {
            ...
            if (!script)
                return res.status(400).json({ error: 'Falta el campo "script"' });
            ...
            if (io)
                io.emit('script_changed', { active: script });
    ...
    router.get('/script/:version', authMiddleware, (req, res) => {
        try {
            ...
            if (version === 'v2')
                filename = 'knowledge_v2.json';
            if (version === 'v3')
                filename = 'knowledge_v3.json';
            if (version === 'v4')
                filename = 'knowledge_v4.json';
    ...
    router.post('/prices', authMiddleware, validate(pricesSchema), (req, res) => {
        try {
            ...
            if (io)
                io.emit('prices_updated', newPrices);
            res.json({ success: true });
        }
        catch (e) {
            res.status(500).json({ error: e.message });
    ...
    router.get('/memory-stats', authMiddleware, async (req, res) => {
        try {
            ...
            if (totalUsers >= DANGER_THRESHOLD)
                recommendation = 'critical';
            else if (totalUsers >= WARN_THRESHOLD)
    ...
    router.post('/reset-memory', authMiddleware, async (req, res) => {
        try {
            ...
            // 3. Notify dashboard
            if (io)
                io.emit('memory_reset', { deletedCount: deletedChats.count });
