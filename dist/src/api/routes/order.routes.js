...
module.exports = (client, sharedState) => {
    ...
    router.put('/orders/:id', authMiddleware, async (req, res) => {
        ...
        if (!bodyResult.success)
            return res.status(400).json({ error: 'Datos inválidos', details: bodyResult.error.issues });
        ...
        try {
            ...
            if (producto !== undefined)
                dataToUpdate.products = producto;
            if (precio !== undefined) {
                const parsed = parseInt(precio.toString().replace(/\./g, '').replace(/[^\d]/g, ''), 10);
                dataToUpdate.totalPrice = isNaN(parsed) ? 0 : parsed;
            }
            if (tracking !== undefined)
                dataToUpdate.tracking = tracking;
            if (status !== undefined)
                dataToUpdate.status = status;
            if (postdatado !== undefined)
                dataToUpdate.postdated = postdatado;
            ...
            if (io)
                io.emit('order_update', legacyOrder);
                ...
                try {
                    ...
                    if (sharedState.userState && sharedState.userState[targetPhone]) {
                        ...
                        if (sharedState.saveState) {
                            try {
                                sharedState.saveState(targetPhone);
                            }
                            catch (e) {
    ...
    router.post('/orders/manual-complete', authMiddleware, async (req, res) => {
        ...
        try {
            ...
            if (alertIndex !== -1) {
                ...
                if (io)
                    io.emit('alerts_updated', sharedState.sessionAlerts);
