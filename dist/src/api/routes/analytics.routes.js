        ...
        try {
            ...
            orders.forEach(order => {
                ...
                if (prodStr.includes('30 d') || prodStr.includes('1 mes') || prodStr.includes('1 f') || prodStr.includes('1 u')) {
                    ...
                    if (/\b30\b/.test(prodStr))
                        duration['30 días']++;
                    else if (/\b60\b/.test(prodStr))
                        duration['60 días']++;
                    else if (/\b90\b/.test(prodStr))
                        duration['90 días']++;
                    else if (/\b120\b/.test(prodStr))
                        duration['120 días']++;
                    else
        ...
        try {
            ...
            if (sharedState?.userState) {
                for (const state of Object.values(sharedState.userState)) {
                    if (state.step)
                        liveStepCounts[state.step] = (liveStepCounts[state.step] || 0) + 1;
                }
            }
            res.json({
                period: snapshots.map(s => {
                    let stepCounts = {};
                    try {
                        stepCounts = JSON.parse(s.stepCounts || '{}');
                    }
                    catch { }
