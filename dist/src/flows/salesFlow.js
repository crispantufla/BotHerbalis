...
exports.processSalesFlow = processSalesFlow;
...
async function processSalesFlow(userId, text, userState, knowledge, dependencies, _recursionDepth = 0) {
    const { saveState } = dependencies;
    // Remove accents and lowercase
    const normalizedText = text.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    // 1. Initialization
    if (!userState[userId]) {
        logger_1.default.info(`[STATE] Initializing new internal state for user ${userId}`);
        userState[userId] = {
        ...
        try {
            ...
            if (existingOrder) {
                logger_1.default.info(`[ORDER-CHECK] User ${userId} has existing order (status: ${existingOrder.status}). Routing to post-sale.`);
            ...
            logger_1.default.error(`[ORDER-CHECK] Failed to query orders for ${userId}:`, err.message);
        }
            ...
            try {
                ...
                if (dbMessages.length === 0 && dependencies.client) {
                    try {
                        ...
                        if (chat) {
                            ...
                            logger_1.default.info(`[SMART-DETECT] DB vacío para ${userId}. Recuperados ${waMapped.length} msjs nativos de WhatsApp.`);
                        }
                    }
                    catch (waErr) {
                        logger_1.default.warn(`[SMART-DETECT] Error recuperando historial nativo WA de ${userId}: ${waErr.message}`);
                    }
                }
                // Check for existence of any prior post-sale outgoing message
                const outgoingMessages = dbMessages.filter((m) => m.role === 'bot' || m.role === 'admin' || m.role === 'system');
                const hasPostSaleMessage = outgoingMessages.some((m) => {
                    const body = (m.content || '').trim().toUpperCase();
                    if (body.includes('MENSAJE DE HERBALIS') || body.includes('MENSAJDE DE HERBALIS'))
                        return true;
                    if (body.includes('CONFIRMACIÓN DE ENVÍO') || body.includes('CONFIRMACION DE ENVIO'))
                        return true;
                    if (body.includes('PEDIDO INGRESADO'))
                        return true;
                    if (/^CO\d{9}$/i.test(body))
                        return true;
                    return false;
                });
                if (hasPostSaleMessage) {
                    logger_1.default.info(`[POST-SALE] User ${userId} has post-sale messages in local DB. Auto-pausing.`);
                    await (0, pauseService_1.pauseUser)(userId, '📦 Cliente post-venta (historial en DB)', { sharedState: dependencies.sharedState, notifyAdmin: dependencies.notifyAdmin }, `El usuario tiene mensajes post-venta en el historial. No ha iniciado conversación nueva.`);
                    return { matched: true, paused: true };
                }
                ...
                if (hasSignificantHistory) {
                    ...
                    if (showsPurchaseIntent) {
                        logger_1.default.info(`[SMART-DETECT] User ${userId} has prior history (outgoing bot: ${outgoingMessages.length}) but shows purchase intent. Allowing sales flow.`);
                    }
                    else {
                        const reason = `😴 Cliente con historial extenso (${outgoingMessages.length}+ mensajes)`;
                        logger_1.default.info(`[SMART-DETECT] User ${userId}: has ${outgoingMessages.length} msgs and NO purchase intent. Auto-pausing.`);
                        await (0, pauseService_1.pauseUser)(userId, reason, { sharedState: dependencies.sharedState, notifyAdmin: dependencies.notifyAdmin }, `${outgoingMessages.length} mensajes previos. Volvió a escribir: "${text.substring(0, 100)}"`);
                        return { matched: true, paused: true };
                    }
                }
                else if (outgoingMessages.length > 0) {
                    logger_1.default.info(`[SMART-DETECT] User ${userId} has ${outgoingMessages.length} prior message(s) (< 10 threshold) in DB. Treating as active prospect.`);
                }
            }
            catch (err) {
                logger_1.default.error(`[SMART-DETECT] Failed to fetch local chat history DB for ${userId}:`, err.message);
            }
        }
    }
    saveState(userId);
    ...
    if (currentState.step === 'completed' || currentState.step === 'cross_bot_redirected') {
        ...
        if (!isAlreadyPaused && currentState.step === 'completed') {
            await (0, pauseService_1.pauseUser)(userId, '👤 Cliente post-venta', { sharedState: dependencies.sharedState, notifyAdmin: dependencies.notifyAdmin }, `El cliente ya compró y volvió a escribir.\n\nMensaje: "${text}"\n\nEl bot se pausó automáticamente.`);
        }
        else {
            logger_1.default.info(`[HARD STOP] User ${userId} is past customer/redirected. Already paused or redirected. Ignoring silently.`);
        }
        return { matched: true, paused: true };
    }
    ...
    if ((0, flowHelpers_1._extractUserName)(normalizedText, currentState)) {
        logger_1.default.info(`[NAME] Detected userName for ${userId}: "${currentState.userName}"`);
        saveState(userId);
    }
    ...
    if (activeSteps.includes(currentState.step)) {
        const extraction = (0, flowHelpers_1._extractSilentVariables)(normalizedText, currentState);
        if (extraction.ageUpdated || extraction.weightUpdated) {
            ...
            if (extraction.isSolelyCorrection) {
                logger_1.default.info(`[GLOBAL EXTRACTION] Intercepted sole correction for ${userId}. Age:${extraction.ageUpdated}, Weight:${extraction.weightUpdated}`);
                let ackMsg = "¡Anotado! 😊\n\nEntonces, decime...";
    ...
    const globalsResult = await (0, globals_1.processGlobals)(userId, text, normalizedText, currentState, knowledge, dependencies);
    if (globalsResult && globalsResult.matched) {
        return; // Handled globally!
    }
    // 3. Process Specific Step Logic
    const stepResult = await (0, steps_1.processStep)(userId, text, normalizedText, currentState, knowledge, dependencies);
    ...
    if (stepResult && stepResult.staleReprocess) {
        if (_recursionDepth >= 2) {
            logger_1.default.error(`[SAFETY] Max recursion depth reached for ${userId} at step "${currentState.step}". Pausing.`);
            await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, `Recursion limit: step "${currentState.step}" no pudo resolverse tras ${_recursionDepth} intentos.`);
            return { matched: true, paused: true };
        }
    ...
    if (!stepResult || !stepResult.matched) {
        logger_1.default.info(`[PAUSE] No match for user ${userId} at step "${currentState.step}". Pausing and alerting admin.`);
        await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, `Bot no pudo responder en paso "${currentState.step}".`);
        return { matched: true, paused: true };
    }
    ...
    if (currentState.history && currentState.history.length > 0) {
        ...
        if (lastHistory.role === 'bot') {
            ...
            if (botMsg.includes('por precaución no recomendamos el consumo') || botMsg.includes('por precaución no recomendamos el uso durante')) {
                logger_1.default.info(`[AI MEDICAL REJECT] Intercepted AI rejection for user ${userId}. Halting flow.`);
                (0, flowHelpers_1._setStep)(currentState, state_1.FlowStep.REJECTED_MEDICAL);
                saveState(userId);
            }
            if (botMsg.includes('Por falta de respeto damos por terminada la comunicación')) {
                logger_1.default.info(`[ABUSE REJECT] Intercepted AI abuse rejection for user ${userId}. Halting flow.`);
                await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, 'El cliente insultó al bot y fue bloqueado automáticamente.');
                saveState(userId);
            }
            if (botMsg.includes('Pensalo tranquilo y cuando estés 100% segura retomamos el pedido')) {
                logger_1.default.info(`[INDECISION PAUSE] Intercepted AI indecision limit for user ${userId}. Halting flow.`);
                await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, 'El cliente cruzó el umbral de indecisión/cambios. Pausa preventiva.');
                saveState(userId);
            }
            if (botMsg.includes('Voy a derivar tu caso a un asesor')) {
                logger_1.default.info(`[CANCEL PAUSE] Intercepted cancel/complaint for user ${userId}. Halting flow.`);
                await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, 'El cliente desea cancelar, reclamar o derivar el caso a un humano.');
                saveState(userId);
            }
            if (botMsg.includes('3413755757') || botMsg.includes('Horacio')) {
                logger_1.default.info(`[RESELLER PAUSE] Intercepted reseller intent for user ${userId}. Halting flow.`);
                await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, 'El cliente está interesado en reventa/compras por mayor. Derivado a Horacio.');
                saveState(userId);
            }
        }
    }
