...
exports.handleSystemGlobals = handleSystemGlobals;
...
async function handleSystemGlobals(userId, text, normalizedText, currentState, dependencies) {
    ...
    const isNegative = (0, validation_1._isNegative)(normalizedText);
    ...
    if (currentState.pendingCancelConfirm && currentState.step !== 'completed') {
        ...
        if (confirmYes) {
            ...
            saveState(userId);
            await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, '🚫 Cancelación confirmada por el cliente durante el proceso de venta.');
            return { matched: true };
        }
        else if (confirmNo) {
            currentState.pendingCancelConfirm = false;
    ...
    if (((EXPLICIT_CANCEL_REGEX.test(normalizedText) && !NO_CANCEL_PHRASE.test(normalizedText))
        ...
        logger_1.default.info(`[GLOBAL] User ${userId} requested cancellation.`);
    ...
    if ((MEDICAL_REJECT_REGEX.test(normalizedText) && !isNegative) || currentState.step === 'rejected_medical') {
        logger_1.default.info(`[MEDICAL REJECT] User ${userId} mentioned contraindicated condition or is already rejected.`);
        ...
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
        (0, flowHelpers_1._setStep)(currentState, 'rejected_medical');
        saveState(userId);
        return { matched: true };
    }
    ...
    if (ABUSIVE_REGEX.test(normalizedText) && currentState.step !== 'rejected_abusive') {
        logger_1.default.info(`[ABUSIVE REJECT] User ${userId} used aggressive language.`);
        ...
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
        (0, flowHelpers_1._setStep)(currentState, 'rejected_abusive');
        await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, '🚨 Lenguaje agresivo o acusación de estafa detectado. Bot silenciado.');
        saveState(userId);
        return { matched: true };
    }
    ...
    if (GEO_REGEX.test(normalizedText) && !currentState.geoRejected && !isCollectingAddress) {
        logger_1.default.info(`[GEO REJECT] User ${userId} is outside Argentina: "${text}"`);
        ...
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
        (0, flowHelpers_1._setStep)(currentState, 'rejected_geo');
        saveState(userId);
        return { matched: true };
    }
    if (currentState.geoRejected || currentState.step === 'rejected_geo') {
        logger_1.default.info(`[GEO REJECT] User ${userId} already geo-rejected, blocking.`);
    ...
    const isTaggedClient = currentState.tags?.some((tag) => tag.name === 'Cliente') ?? false;
    ...
    if (isTaggedClient || historyIndicatesSale) {
        logger_1.default.info(`[CLIENT SUPPORT] User ${userId} is an existing client speaking. Pausing bot.`);
        await (0, flowHelpers_1._pauseAndAlert)(userId, currentState, dependencies, text, '🚨 Cliente recurrente o con compra reciente escribiendo. Intervención humana requerida.');
        return { matched: true };
    }
    // 4. CHANGE ORDER
    ...
    if (CHANGE_REGEX.test(normalizedText) && currentState.step !== 'greeting' && currentState.step !== 'waiting_data' && !isNegative) {
        logger_1.default.info(`[GLOBAL] User ${userId} requested change.`);
        currentState.cart = [];
        ...
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        await sendMessageWithDelay(userId, msg);
        (0, flowHelpers_1._setStep)(currentState, 'waiting_preference');
        saveState(userId);
        return { matched: true };
    }
