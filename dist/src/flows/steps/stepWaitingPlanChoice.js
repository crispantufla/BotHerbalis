...
exports.handleWaitingPlanChoice = handleWaitingPlanChoice;
...
function _handleExtractedData(userId, extractedData, currentState) {
    if (!extractedData || extractedData === 'null')
        return;
    logger_1.default.info(`[DATA EXTRACTION] User ${userId}: ${extractedData}`);
    if (extractedData.startsWith('PROFILE:')) {
        ...
        logger_1.default.info(`[PROFILE SAVED] ${currentState.profile}`);
    }
        ...
        logger_1.default.info(`[POSTDATADO SAVED] Fecha: ${fecha}`);
    }
}
async function handleWaitingPlanChoice(userId, text, normalizedText, currentState, knowledge, dependencies) {
    ...
    for (const part of parts) {
        if (!part || part.trim().length < 3)
            continue;
        let p = null;
        let pl = null;
        for (const prod of products)
            if (prod.match.test(part))
                p = prod.name;
        for (const plan of plans)
            if (plan.match.test(part))
                pl = plan.id;
        if (p && pl) {
            foundItems.push({
                product: p,
                plan: pl,
                price: (0, pricing_1._getPrice)(p, pl)
            });
        }
    }
    if (foundItems.length > 0) {
        ...
        currentState.adicionalMAX = applyMax ? (0, pricing_1._getAdicionalMAX)() : 0;
        currentState.cart = foundItems;
        const closingNode = knowledge.flow.closing;
        (0, flowHelpers_1._setStep)(currentState, closingNode.nextStep);
    ...
    if (selectedPlanId) {
        ...
        (0, cartHelpers_1.buildCartFromSelection)(product, selectedPlanId, currentState);
        planSelected = true;
    }
    if (planSelected) {
        const closingNode = knowledge.flow.closing;
        ...
        if (hasAddress) {
            logger_1.default.info(`[FLOW-SKIP] Address already collected for ${userId}, skipping data request.`);
        ...
        if (isAffirmative && aiRecommended120) {
            logger_1.default.info(`[FLOW-INTERCEPT] User said OK to 120-day plan upsell/AI recommendation: ${userId}`);
            const product = currentState.selectedProduct || "Nuez de la India";
            (0, cartHelpers_1.buildCartFromSelection)(product, '120', currentState);
            const closingNode = knowledge.flow.closing;
            const addr = currentState.partialAddress || {};
            const hasAddress = addr.nombre && addr.calle && addr.ciudad;
            if (hasAddress) {
                logger_1.default.info(`[FLOW-SKIP] Address already collected for ${userId}, skipping data request after upsell.`);
            ...
            logger_1.default.info(`[AI-FALLBACK] waiting_plan_choice: No plan number detected for ${userId}`);
...
8) HORARIO DE ENVÍO: Si pregunta cuándo o a qué hora llega, aclará que Correo Argentino maneja su propia logística y no podemos asegurar el horario, pero que avisamos si hay que retirar. Luego volvé al plan. goalMet=false.`,
            ...
            if (planAI.extractedData && typeof planAI.extractedData === 'string' && planAI.extractedData.startsWith('CHANGE_PRODUCT:')) {
                ...
                logger_1.default.info(`[FLOW-UPDATE] User changed product to: ${newProd}`);
            ...
            if (planAI.goalMet && planAI.extractedData && !planAI.extractedData.startsWith('CHANGE_PRODUCT:')) {
                ...
                if (extractedStr.startsWith('POSTDATADO:') && currentState.selectedProduct) {
                    ...
                    if (!userChosePlan)
                        userChosePlan = normalizedText.match(/\b(60|120|180|240|300|360|420|480|540|600)\b/);
                    if (userChosePlan) {
                        const plan = userChosePlan[1];
                        const product = currentState.selectedProduct || "Nuez de la India";
                        (0, cartHelpers_1.buildCartFromSelection)(product, plan, currentState);
                        logger_1.default.info(`[FLOW-UPDATE] Saved plan ${plan} along with POSTDATADO.`);
                    }
                    (0, flowHelpers_1._setStep)(currentState, closingNode.nextStep);
                    if (planAI.response) {
                    ...
                    const hasAddress = addr.nombre && addr.calle && addr.ciudad;
                    if (hasAddress) {
                        logger_1.default.info(`[FLOW-SKIP] Address already collected for ${userId}, skipping data request after AI plan.`);
                        if (planAI.response) {
                            currentState.history.push({ role: 'bot', content: planAI.response, timestamp: Date.now() });
                            await sendMessageWithDelay(userId, planAI.response);
                        }
                    ...
                    logger_1.default.warn(`[AI-SAFEGUARD] waiting_plan_choice: AI returned goalMet=true but no 60/120/180/240 etc in extractedData (${extractedStr}). Downgrading to false.`);
                    if (planAI.response) {
                        currentState.history.push({ role: 'bot', content: planAI.response, timestamp: Date.now() });
                        await sendMessageWithDelay(userId, planAI.response);
                        saveState(userId);
                ...
                if ((0, messages_1._isDuplicate)(planAI.response, currentState.history)) {
                    logger_1.default.info(`[ANTI-DUP] Skipping duplicate AI response for ${userId} in plan_choice`);
                ...
                if (planAI.response.includes('después hablamos') || planAI.response.includes('cualquier cosa acá estoy')) {
                    ...
                    logger_1.default.info(`[FLOW] User ${userId} delayed step. Will follow up later.`);
                }
                return { matched: true };
            }
        }
