...
exports.handleWaitingPreference = handleWaitingPreference;
...
async function handleWaitingPreference(userId, text, normalizedText, currentState, knowledge, dependencies) {
    ...
    const earlyPostdatado = (0, flowHelpers_1._detectPostdatado)(normalizedText);
    if (earlyPostdatado) {
        logger_1.default.info(`[EARLY POSTDATADO] Captured in waiting_preference: ${earlyPostdatado}`);
        if (!currentState.postdatado)
            currentState.postdatado = earlyPostdatado;
        saveState(userId);
    }
    ...
    const adaptResponsePrefix = (rawResponse, userText, extString) => {
    ...
    if (isComparison) {
        if (/^(capsulas? o gotas?|gotas? o capsulas?|capsulas o gotas porfa(?:vor)?)$/i.test(normalizedText.trim())) {
            logger_1.default.info(`[HARDCODED-PREF] User asked "capsulas o gotas", sending hardcoded recommendation.`);
        ...
        logger_1.default.info(`[INDICISION] User ${userId} compares products or asks for recommendation.`);
        const aiRecommendation = await aiService.chat(text, {
            step: state_1.FlowStep.WAITING_PREF_CONSULT,
        ...
        if (aiRecommendation.goalMet && aiRecommendation.extractedData) {
            ...
            if (priceNode) {
                ...
                const msg = (0, messages_1._formatMessage)(adaptedResponse, currentState);
    ...
    if (mentionsCapsulas) {
        currentState.selectedProduct = "Cápsulas de nuez de la india";
        const msg = (0, messages_1._formatMessage)(knowledge.flow.preference_capsulas.response, currentState);
        (0, flowHelpers_1._setStep)(currentState, knowledge.flow.preference_capsulas.nextStep);
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        saveState(userId);
        await sendMessageWithDelay(userId, msg);
        await (0, flowHelpers_1._maybeUpsell)(currentState, sendMessageWithDelay, userId, saveState);
        ...
        currentState.selectedProduct = "Semillas de nuez de la india";
        const msg = (0, messages_1._formatMessage)(knowledge.flow.preference_semillas.response, currentState);
        (0, flowHelpers_1._setStep)(currentState, knowledge.flow.preference_semillas.nextStep);
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        saveState(userId);
        await sendMessageWithDelay(userId, msg);
        await (0, flowHelpers_1._maybeUpsell)(currentState, sendMessageWithDelay, userId, saveState);
        ...
        currentState.selectedProduct = "Gotas de nuez de la india";
        const msg = (0, messages_1._formatMessage)(knowledge.flow.preference_gotas.response, currentState);
        (0, flowHelpers_1._setStep)(currentState, knowledge.flow.preference_gotas.nextStep);
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        saveState(userId);
        await sendMessageWithDelay(userId, msg);
        await (0, flowHelpers_1._maybeUpsell)(currentState, sendMessageWithDelay, userId, saveState);
        ...
        logger_1.default.info(`[AI-FALLBACK] waiting_preference: No keyword match for ${userId}`);
        const aiPref = await aiService.chat(text, {
            step: state_1.FlowStep.WAITING_PREFERENCE,
