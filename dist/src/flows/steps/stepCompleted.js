...
exports.handleCompleted = handleCompleted;
...
async function handleCompleted(userId, text, normalizedText, currentState, knowledge, dependencies) {
    ...
    logger_1.default.info(`[POST-SALE] Message from completed customer ${userId}: "${text}"`);
    ...
    if (currentState.pendingCancelConfirm) {
        if ((0, validation_1._isAffirmative)(normalizedText)) {
    ...
    if (TRACKING_REGEX.test(normalizedText)) {
        logger_1.default.info(`[POST-SALE] [REGEX] Tracking query detected from ${userId}: "${text}". Auto-pausing.`);
    ...
    if (COMPLAINT_REGEX.test(normalizedText)) {
        logger_1.default.info(`[POST-SALE] [REGEX] Complaint detected from ${userId}: "${text}". Auto-pausing.`);
        if (dependencies.sharedState?.pausedUsers) {
            const { pauseUser } = require('../../services/pauseService');
...
7. NUNCA inventes información. NUNCA pidas datos de envío ni intentes venderle cápsulas/semillas/gotas sin que lo pida.`,
    ...
    if (postSaleAI.aiUnavailable) {
        logger_1.default.info(`[POST-SALE] AI unavailable for ${userId}. Auto-pausing for admin.`);
        if (dependencies.sharedState?.pausedUsers) {
            const { pauseUser } = require('../../services/pauseService');
    ...
    if (postSaleAI.extractedData === 'TRACKING_INFO') {
        logger_1.default.info(`[POST-SALE] Customer ${userId} is asking for tracking/shipping info. Auto-pausing silently.`);
        if (dependencies.sharedState?.pausedUsers) {
            const { pauseUser } = require('../../services/pauseService');
            await pauseUser(userId, '📦 Consulta de tracking post-venta', { sharedState: dependencies.sharedState });
        }
        ...
        logger_1.default.info(`[POST-SALE] Customer ${userId} wants to re-purchase. Skipping to preference.`);
        ...
        logger_1.default.info(`[POST-SALE] Customer ${userId} wants to cancel. Setting pendingCancelConfirm and asking for confirmation.`);
        ...
        logger_1.default.info(`[POST-SALE] Customer ${userId} wants to post-date delivery to: ${newDate}`);
