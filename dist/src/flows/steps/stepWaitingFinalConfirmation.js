...
exports.handleWaitingFinalConfirmation = handleWaitingFinalConfirmation;
...
async function handleWaitingFinalConfirmation(userId, text, normalizedText, currentState, knowledge, dependencies) {
    ...
    if ((productChangeMatch || planChangeMatch) && currentState.selectedPlan) {
        ...
        if (newProduct !== currentState.selectedProduct || newPlan !== currentState.selectedPlan) {
            ...
            const priceStr = (0, pricing_1._getPrice)(newProduct, newPlan);
            ...
            if (currentState.isContraReembolsoMAX) {
                finalAdicional = newPlan === '60' ? (0, pricing_1._getAdicionalMAX)() : 0;
            }
    ...
    if (currentState.postdatado && (0, validation_1._isAffirmative)(normalizedText)) {
        ...
        if (currentState.pendingOrder) {
            ...
            if (dependencies.saveOrderToLocal)
                dependencies.saveOrderToLocal(orderData);
        ...
        (0, flowHelpers_1._setStep)(currentState, state_1.FlowStep.WAITING_ADMIN_VALIDATION);
        currentState.history.push({ role: 'bot', content: msg, timestamp: Date.now() });
        saveState(userId);
        return { matched: true };
    }
    else if ((0, validation_1._isAffirmative)(normalizedText) || /\b(si|dale|ok|listo|confirmo|correcto|acepto|bueno|joya|de una)\b/i.test(normalizedText)) {
        const msg = "¡Perfecto! Recibimos tu confirmación.\n\nAguardame un instante que verificamos los datos y te confirmamos el ingreso ⏳";
        await sendMessageWithDelay(userId, msg);
        if (currentState.pendingOrder) {
        ...
        logger_1.default.info(`[AI-FALLBACK] waiting_final_confirmation: Delegando consulta a IA para ${userId}`);
        ...
        logger_1.default.warn(`[FINAL_CONFIRM] Unrecognized response from ${userId}: "${text}" — pausing for admin review`);
