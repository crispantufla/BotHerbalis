...
async function _pauseAndAlert(userId, currentState, dependencies, userMessage, reason) {
    ...
    if (notifyAdmin) {
        await notifyAdmin(`🚨 BOT PAUSADO${nightLabel} — Necesita intervención`, userId, `Razón: ${reason}\nÚltimo mensaje: "${userMessage}"\nPaso: ${currentState.step}${suggestionsText}`);
    }
    ...
    logger_1.default.info(`⏸️ [BOT] User ${userId} paused. Reason: ${reason}${nightLabel}`);
}
/**
 * _extractUserName
...
function _extractUserName(normalizedText, currentState) {
    ...
    const nameMatch = normalizedText.match(/\b(?:soy|me\s+llamo|mi\s+nombre\s+es|llamame|me\s+dicen)\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]{2,}(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]{2,})?)\b/i);
...
function _resolveNewProductPlan(normalizedText, currentProduct, currentPlan) {
    ...
    if (/capsula|pastilla/i.test(normalizedText))
        newProduct = "Cápsulas de nuez de la india";
