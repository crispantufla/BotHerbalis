...
exports._formatPrice = _formatPrice;
exports._recalcAdicionalMAX = _recalcAdicionalMAX;
exports.buildCartFromSelection = buildCartFromSelection;
exports.buildMultiProductCart = buildMultiProductCart;
 ...
 */
function _recalcAdicionalMAX(state) {
    const totalDays = state.cart.reduce((sum, item) => {
        return sum + parseInt(item.plan || '0', 10);
    }, 0);
    if (totalDays < 120) {
        state.isContraReembolsoMAX = true;
        state.adicionalMAX = (0, pricing_1._getAdicionalMAX)();
    }
    else {
 ...
 */
function buildCartFromSelection(product, plan, state) {
    const planDays = parseInt(plan, 10);
    const raw120 = (0, pricing_1._getPrice)(product, '120');
    const raw60 = (0, pricing_1._getPrice)(product, '60');
    ...
    if (isNaN(base120) || isNaN(base60) || base60 === 0) {
        ...
        const err = new Error(`Precio inválido para el producto "${product}". Verificar prices.json.`);
 ...
 */
function buildMultiProductCart(items, state) {
    if (items.length === 0)
        return;
    const logger = require('../../utils/logger');
    const totalUnits = items.reduce((sum, i) => sum + i.units, 0);
    const itemPrices = items.map(item => {
        const raw60 = (0, pricing_1._getPrice)(item.product, '60');
        const raw120 = (0, pricing_1._getPrice)(item.product, '120');
        const base60 = parseInt((raw60 || '0').replace(/\./g, ''), 10);
        const base120 = parseInt((raw120 || '0').replace(/\./g, ''), 10);
        if (base60 === 0) {
    ...
    const cartPrices = itemPrices.map(item => {
