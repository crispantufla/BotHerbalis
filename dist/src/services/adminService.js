...
exports.parseAdminInput = parseAdminInput;
exports.resolveAlertTarget = resolveAlertTarget;
exports.notifyAdmin = notifyAdmin;
exports.buildAdminApprovalMessage = buildAdminApprovalMessage;
exports.handleAdminCommand = handleAdminCommand;
const child_process_1 = require("child_process");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const ai_1 = require("./ai");
const messages_1 = require("../flows/utils/messages");
...
function _dismissAlert(userPhone, sharedState) {
    const before = sharedState.sessionAlerts.length;
    sharedState.sessionAlerts = sharedState.sessionAlerts.filter((a) => a.userPhone !== userPhone);
    if (sharedState.sessionAlerts.length !== before) {
        if (sharedState.io)
            sharedState.io.emit('alerts_updated', sharedState.sessionAlerts);
    }
}
/**
...
function parseAdminInput(text) {
    ...
    if (qrMatch)
        return { selector: qrMatch[1], command: qrMatch[2].toLowerCase() };
    // Match: starts with 1-2 digit number, then a space, then the rest
    const match = trimmed.match(/^(\d{1,2})\s+(.+)$/);
    if (match)
        return { selector: match[1], command: match[2].trim() };
 ...
 */
function resolveAlertTarget(selector, targetChatId, sharedState) {
...
async function notifyAdmin(reason, userPhone, details = null, sharedState, client, config) {
    if (process.platform === 'win32') {
        (0, child_process_1.exec)('powershell "[console]::beep(1000, 500)"', (err) => { if (err)
            logger_1.default.error('Beep failed:', err); });
    }
    logger_1.default.info(`[ADMIN ALERT] ${reason} (User: ${userPhone})`);
    const now = Date.now();
    ...
    const lastAlert = sharedState.sessionAlerts.find((a) => a.userPhone === userPhone && a.reason === reason);
    if (lastAlert && (now - lastAlert.id < 8000))
        return;
    ...
    const lastUserMsg = state.history?.filter((h) => h.role === 'user').pop()?.content || '';
    const quickReplies = (0, messages_1._getQuickReplies)(state.step || '', lastUserMsg);
    const newAlert = {
        id: Date.now(),
        timestamp: new Date(),
        reason,
    ...
    if (sharedState.sessionAlerts.length > 50)
        sharedState.sessionAlerts.pop();
    if (sharedState.io)
        sharedState.io.emit('new_alert', newAlert);
    if (config.alertNumbers && config.alertNumbers.length > 0) {
        ...
        for (const num of config.alertNumbers) {
            ...
            client.sendMessage(targetAlert, alertMsg).catch((e) => logger_1.default.error(`[ALERT] Failed to forward to ${num}:`, e.message));
        }
    }
}
// Helper: Build the WhatsApp confirmation sent to client after admin approves
function buildAdminApprovalMessage(clientState) {
    if (!clientState.pendingOrder)
    ...
    const planDays = clientState.selectedPlan
...
async function handleAdminCommand(targetChatId, commandText, isApi = false, sharedState, client, alertSelector = null) {
    if (!commandText)
    ...
    if (lowerMsg.startsWith('!despauser ') || lowerMsg.startsWith('!reanudar ') || lowerMsg.startsWith('!unpause ')) {
        ...
        if (!targetNum)
            return '⚠️ Falta el teléfono. Ejemplo: !despauser 5491155551234';
        ...
        if (sharedState.io)
            sharedState.io.emit('bot_status_change', { chatId: targetChat, paused: false });
        logger_1.default.info(`[ADMIN] Unpaused ${targetChat} via WhatsApp command.`);
    ...
    if (lowerMsg.startsWith('!pedido')) {
        try {
            ...
            const where = { instanceId: INSTANCE_ID };
            if (phoneArg)
                where.userPhone = { contains: phoneArg };
            ...
            if (orders.length === 0)
                return phoneArg ? `⚠️ No hay pedidos para ${phoneArg}.` : '⚠️ No hay pedidos recientes.';
    ...
    if (lowerMsg.startsWith('!tracking ')) {
        ...
        try {
            ...
            if (!order)
                return `⚠️ No hay pedido activo para ${phoneArg}.`;
            ...
            if (sharedState.logAndEmit)
                sharedState.logAndEmit(targetChat, 'bot', msg, 'tracking_sent');
            if (sharedState.io)
                sharedState.io.emit('order_update', { action: 'updated', order: { id: order.id, tracking: trackingCode } });
            logger_1.default.info(`[ADMIN] Tracking updated for ${phoneArg}: ${trackingCode}`);
    ...
    if (lowerMsg.startsWith('!reset ')) {
        ...
        const targetNum = parts[1];
        if (!targetNum)
            return '⚠️ Falta el teléfono. Ejemplo: !reset 5491155551234';
        const targetChat = targetNum.includes('@') ? targetNum : `${targetNum.replace(/\D/g, '')}@c.us`;
        ...
        try {
            ...
            logger_1.default.warn(`[ADMIN] Could not clear DB state for ${targetNum}:`, e.message);
        }
        _dismissAlert(targetChat, sharedState);
        if (sharedState.logAndEmit)
            sharedState.logAndEmit(targetChat, 'system', 'Memoria reiniciada por admin', 'new');
        if (sharedState.io)
            sharedState.io.emit('bot_status_change', { chatId: targetChat, paused: false });
        logger_1.default.info(`[ADMIN] Reset user ${targetChat} via WhatsApp command.`);
    ...
    if (lowerMsg.startsWith('!pausa-global') || lowerMsg.startsWith('!global')) {
        ...
        if (sharedState.saveState)
            sharedState.saveState();
        if (sharedState.io)
            sharedState.io.emit('global_pause_changed', { globalPause: sharedState.config.globalPause });
        logger_1.default.info(`[ADMIN] Global pause toggled to: ${sharedState.config.globalPause}`);
    ...
    if (lowerMsg === '!precios' || lowerMsg === '!precio' || lowerMsg === '!prices') {
        try {
            ...
            const PRICES_FILE = path_1.default.join(DATA_DIR, 'prices.json');
            if (!fs_1.default.existsSync(PRICES_FILE))
                return '⚠️ Archivo de precios no encontrado.';
            const prices = JSON.parse(fs_1.default.readFileSync(PRICES_FILE, 'utf8'));
            const lines = [];
            for (const [product, plans] of Object.entries(prices)) {
                const planStr = Object.entries(plans)
    ...
    if (lowerMsg.startsWith('!historial ') || lowerMsg.startsWith('!historia ')) {
        ...
        const targetNum = parts[1];
        if (!targetNum)
            return '⚠️ Falta el teléfono. Ejemplo: !historial 5491155551234';
        const targetChat = targetNum.includes('@') ? targetNum : `${targetNum.replace(/\D/g, '')}@c.us`;
        const state = sharedState.userState[targetChat] || {};
        const history = state.history || [];
        if (history.length === 0)
            return `⚠️ No hay historial para ${targetNum}.`;
        const historyText = history.slice(-30).map((m) => `${m.role}: ${m.content}`).join('\n');
        try {
            const summary = await ai_1.aiService.generateSuggestion('Hacé un resumen breve de esta conversación para el admin. Incluí: qué producto quiere, en qué paso está, si hay algún problema.', historyText);
            return summary
    ...
    if (lowerMsg.startsWith('!enviar ') || lowerMsg.startsWith('!msg ')) {
        ...
        if (!targetNum || !message)
            return '⚠️ Formato: !enviar [telefono] [mensaje]\nEjemplo: !enviar 5491155551234 Hola, te contactamos desde Herbalis';
        const targetChat = targetNum.includes('@') ? targetNum : `${targetNum.replace(/\D/g, '')}@c.us`;
        try {
            await client.sendMessage(targetChat, message);
            if (sharedState.logAndEmit)
                sharedState.logAndEmit(targetChat, 'admin', message, 'admin_direct');
            logger_1.default.info(`[ADMIN] Direct message sent to ${targetChat}: "${message.substring(0, 50)}..."`);
    ...
    if (lowerMsg === '!funnel') {
        const states = Object.values(sharedState.userState);
        const stepOrder = [
    ...
    if (lowerMsg === '!abandonos') {
        ...
        for (const s of withFollowUp) {
            ...
            if (!groups[key])
                groups[key] = { total: 0, converted: 0, variants: {} };
            groups[key].total++;
            if (fd.converted)
                groups[key].converted++;
            if (!groups[key].variants[fd.variantIndex])
                groups[key].variants[fd.variantIndex] = { total: 0, converted: 0 };
            groups[key].variants[fd.variantIndex].total++;
            if (fd.converted)
                groups[key].variants[fd.variantIndex].converted++;
        }
        const typeLabels = { cold_lead: '❄️ Lead frio', abandoned_cart: '🛒 Carrito abandonado' };
    ...
    if (lowerMsg.startsWith('!script ') || lowerMsg === '!script') {
        ...
        if (sharedState.io)
            sharedState.io.emit('script_changed', { active: version });
        if (sharedState.saveState)
            sharedState.saveState();
        logger_1.default.info(`[ADMIN] Script switched to: ${version} via WhatsApp`);
        ...
        if (amountMatch) {
            ...
            if (!isNaN(amount) && amount > 0) {
                try {
                    ...
                    if (!mpToken)
                        return '⚠️ MP_ACCESS_TOKEN no configurado en .env';
                    ...
                    logger_1.default.info(`[MP] Payment link created for $${amount} ARS: ${link}`);
                    return `✅ Enlace de pago generado:\n💳 $${amount} ARS\n\n${link}`;
                }
                catch (e) {
                    logger_1.default.error('[MP] Error creating preference:', e);
    ...
    if (lowerMsg.includes('me encargo') || lowerMsg.includes('intervenir')) {
        if (!actualTarget)
            return '⚠️ No hay usuario pendiente. Usá "!alertas" para ver la cola.';
        ...
        if (sharedState.saveState)
            sharedState.saveState();
        if (sharedState.io)
            sharedState.io.emit('bot_status_change', { chatId: actualTarget, paused: true });
        _dismissAlert(actualTarget, sharedState);
        const label = _targetLabel(actualTarget);
        logger_1.default.info(`[ADMIN] Takeover for ${actualTarget}. Bot PAUSED.`);
    ...
    if (qrMatch) {
        ...
        const alert = sharedState.sessionAlerts.find((a) => a.userPhone === actualTarget);
        ...
        if (sharedState.logAndEmit)
            sharedState.logAndEmit(actualTarget, 'bot', qr.message, clientState?.step);
        if (sharedState.saveState)
            sharedState.saveState();
        const label = _targetLabel(actualTarget);
        logger_1.default.info(`[ADMIN] Quick reply r${qrIndex + 1} sent to ${actualTarget}: "${qr.label}"`);
    ...
    if (lowerMsg === 'ok' || lowerMsg === 'dale' || lowerMsg === 'si' || lowerMsg === 'confirmar') {
        ...
        if (clientState && clientState.step === 'waiting_admin_ok' && clientState.pendingOrder) {
            ...
            if (sharedState.logAndEmit)
                sharedState.logAndEmit(actualTarget, 'bot', summary, 'waiting_final_confirmation');
        ...
        try {
            ...
            if (existingOrder) {
                ...
                if (sharedState.logAndEmit)
                    sharedState.logAndEmit(actualTarget, 'bot', msg, 'completed');
                if (sharedState.io) {
            ...
            logger_1.default.error('[ADMIN] Error confirming order in DB:', e);
        }
    ...
    if (actualTarget) {
        try {
            ...
            const suggestion = await ai_1.aiService.generateSuggestion(commandText, contextStr);
            if (suggestion) {
                const label = _targetLabel(actualTarget);
                await client.sendMessage(actualTarget, suggestion);
                if (sharedState.logAndEmit)
                    sharedState.logAndEmit(actualTarget, 'admin', suggestion, 'admin_instruction');
                if (sharedState.pausedUsers.has(actualTarget)) {
                    const { unpauseUser: unpauseUserFn } = require('./pauseService');
                    await unpauseUserFn(actualTarget, sharedState);
                    if (sharedState.io)
                        sharedState.io.emit('bot_status_change', { chatId: actualTarget, paused: false });
                }
                _dismissAlert(actualTarget, sharedState);
            ...
            logger_1.default.error('AI Suggestion Error:', e);
