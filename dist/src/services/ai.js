...
function _getRelevantRules(userText) {
...
function _getModelForStep(step) {
...
async function _getPrices() {
    const now = Date.now();
    if (_pricesCache && (now - _pricesCacheTime) < PRICES_CACHE_MS)
        return _pricesCache;
    let prices = {
    ...
    try {
        ...
        logger_1.default.error("Error reading prices for AI:", e.message);
    }
    _pricesCache = prices;
...
function _getCorePrompt(userText = "") {
...
async function _buildSystemPrompt(step, userText = "") {
...
class AIService {
    ...
    constructor() {
        ...
        if (!apiKey) {
            logger_1.default.error("❌ CRITICAL: OPENAI_API_KEY is missing!");
        }
        this._disabled = !apiKey;
        logger_1.default.info(`📡[AI] Initializing OpenAI(base: ${MODEL}, premium: ${MODEL_PREMIUM})`);
        this.client = new openai_1.default({ apiKey, timeout: 15_000 });
        this.model = MODEL;
        this.cache = new node_cache_1.default({ stdTTL: CACHE_TTL_SECONDS, checkperiod: 120, maxKeys: 1000 });
     ...
     */
    async _callQueued(apiCallFn, rawCacheKey = null, customTTL = undefined) {
        if (this._disabled)
            throw new Error('AI Service disabled: missing API key');
        // Check cache first
        let cacheKey = null;
        if (rawCacheKey) {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (isRetryable) {
                    ...
                    logger_1.default.warn(`⚠️[AI] Retryable error (${status || e.code}). Attempt ${attempt + 1}/${MAX_RETRIES}. Backing off ${waitTime / 1000}s...`);
        ...
        if (!success || result === undefined) {
            ...
            if (this._circuitBreaker.failures >= CIRCUIT_BREAKER_THRESHOLD) {
                ...
                logger_1.default.warn(`⚠️[AI] Circuit breaker OPEN — ${CIRCUIT_BREAKER_THRESHOLD} consecutive failures. Cooling down ${CIRCUIT_BREAKER_RESET_MS / 1000}s.`);
            }
    ...
    async chat(userText, context) {
        ...
        if (context.knowledge && context.knowledge.flow) {
            ...
            if (pathInfo)
                knowledgeContext += `- SOBRE PATOLOGÍAS: "${pathInfo}"\n`;
        ...
        if (context.userState) {
            ...
            if (s.selectedProduct)
                stateContext += `- Producto elegido: ${s.selectedProduct} \n`;
...
`;
        try {
            ...
            const result = await this._callQueued(() => this.client.chat.completions.create({
                model: chatModel,
                messages: [
            ...
            logger_1.default.warn("⚠️[AI] No tool_calls in response. Returning aiUnavailable.");
            return { response: null, goalMet: false, aiUnavailable: true };
        }
        catch (e) {
            logger_1.default.error("🔴 [AI] Chat Error:", e.message);
            return { response: null, goalMet: false, aiUnavailable: true };
        }
    }
    /**
    ...
    async checkAndSummarize(history) {
        if (!history || history.length <= 80)
            return null;
        logger_1.default.info(`[AI] Summarizing history (${history.length} messages down to ${MAX_HISTORY_LENGTH})...`);
        const summary = await this._callQueuedSummarize(history.slice(0, -MAX_HISTORY_LENGTH));
        if (summary) {
            logger_1.default.info(`[AI] Summary created: "${summary.substring(0, 50)}..."`);
            return {
                summary,
    ...
    async _callQueuedSummarize(history) {
        const conversationText = history.map(msg => `${msg.role === 'user' ? 'Cliente' : 'Vendedor'}: ${msg.content} `).join('\n');
...
`;
        try {
            ...
            logger_1.default.error("🔴 [AI] Summary Error:", e.message);
            return null;
        }
    }
    /**
    ...
    async generateReport(prompt) {
        ...
        try {
            ...
            logger_1.default.error("🔴 [AI] Report Error:", e.message);
            throw e;
        }
    }
    /**
...
1. Tu prioridad es extraer CUALQUIER dato útil, aunque falten otros.
        ...
        try {
            ...
            logger_1.default.error("🔴 [AI] parseAddress Error:", e.message);
            return { _error: true };
        }
    }
    /**
    ...
    async transcribeAudio(mediaData, mimeType) {
        ...
        try {
            ...
            const result = await this._callQueued(() => this.client.audio.transcriptions.create({
                model: "whisper-1",
            ...
            logger_1.default.error("🔴 [AI] Transcribe Error:", e.message);
            return null;
        }
        finally {
            try {
    ...
    async analyzeImage(mediaData, mimeType, prompt) {
        try {
            ...
            logger_1.default.error("🔴 [AI] Vision Error:", e.message);
            return null;
        }
    }
    /**
...
SITUACION: El ADMINISTRADOR del negocio te da una instrucción DIRECTA para enviarle al cliente.
     ...
     */
    async generateContextualBridge(userMessage, context) {
        const prompt = `
...
1. NO hagas ninguna pregunta.
        ...
        try {
            const result = await this._callQueued(() => this.client.chat.completions.create({
                ...
                max_tokens: 40
            }), `bridge_${userMessage} `, 60 * 60 // 1 hour cache to avoid unnecessary calls for common phrases
            );
            const bridge = result.choices[0].message?.content?.trim() || "";
            return bridge;
        }
        catch (e) {
            logger_1.default.error("🔴 [AI] Contextual Bridge Error:", e.message);
