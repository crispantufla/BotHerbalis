...
exports.checkStaleUsers = checkStaleUsers;
exports.checkColdLeads = checkColdLeads;
exports.checkAbandonedCarts = checkAbandonedCarts;
exports.autoApproveOrders = autoApproveOrders;
exports.cleanStalePausedUsers = cleanStalePausedUsers;
exports.snapshotDailyStats = snapshotDailyStats;
 ...
 */
function _detectAbandonReason(state) {
    if (state.addressIssueType)
    ...
    if (/cobro|sueldo|plata|quincena|depositan|me pagan|sin guita|no tengo la plata|no me alcanza/.test(t))
        return 'payment_timing';
    if (/pensar|despues|luego|manana|te aviso|te confirmo|mas tarde|semana que viene|mes que viene/.test(t))
        return 'hesitation';
    if (/resultado|miedo|seguro|funciona|garantia|duda|probar|riesgo|caro|precio|sale|vale/.test(t))
        return 'objection';
    return 'generic';
}
/**
...
function checkStaleUsers(sharedState, dependencies) {
    ...
    for (const [userId, state] of Object.entries(userState)) {
        if (!state.step || state.step === 'completed' || state.step === 'greeting')
            continue;
        if (state.step === 'waiting_admin_ok')
            continue;
        if (pausedUsers && pausedUsers.has(userId))
            continue;
        if (state.staleAlerted)
            continue;
        if (!state.stepEnteredAt)
            continue;
        const minutes = (0, date_fns_1.differenceInMinutes)(now, state.stepEnteredAt);
        if (minutes > STALE_THRESHOLD_MINS) {
            logger_1.default.info(`[SCHEDULER] Stale user detected: ${userId} on step "${state.step}" for ${minutes} min`);
            notifyAdmin(`⏰ Cliente estancado ${minutes} min`, userId, `Paso: ${state.step}\nÚltima actividad: hace ${minutes} min\nProducto: ${state.selectedProduct || '?'}`).catch(e => logger_1.default.error('[SCHEDULER] notifyAdmin error:', e.message));
 ...
 */
async function autoApproveOrders(sharedState, dependencies) {
    if (_autoApproveRunning) {
        logger_1.default.info('[SCHEDULER] autoApproveOrders already running, skipping.');
        return;
    }
    ...
    try {
        ...
        for (const [userId, state] of Object.entries(userState)) {
            ...
            if (minutes > AUTO_APPROVE_THRESHOLD_MINS) {
                logger_1.default.info(`[AUTO-APPROVE] Order for ${userId} auto-approved after ${minutes} min without admin review`);
                const confirmMsg = (0, messageTemplates_1.buildConfirmationMessage)(state);
                try {
                    await sendMessageWithDelay(userId, confirmMsg);
                }
                catch (e) {
                    logger_1.default.error(`[AUTO-APPROVE] Failed to send confirmation to ${userId}:`, e.message);
                    continue;
                }
                ...
                saveState(userId);
                notifyAdmin('⚡ Pedido AUTO-APROBADO (15 min sin revisión)', userId, `El pedido fue aprobado automáticamente.\nProducto: ${state.cart ? state.cart.map(i => i.product).join(' + ') : '?'}\nTotal: $${state.totalPrice || '?'}\n⚠️ Revisar en panel de ventas.`).catch(e => logger_1.default.error('[SCHEDULER] Auto-approve notify error:', e.message));
            }
        }
    }
    finally {
 ...
 */
async function checkColdLeads(sharedState, dependencies) {
    const { userState, pausedUsers } = sharedState;
    const { sendMessageWithDelay, saveState } = dependencies;
    const now = Date.now();
    for (const [userId, state] of Object.entries(userState)) {
        if (!RE_ENGAGEABLE_STEPS.has(state.step))
            continue;
        if (!(0, timeUtils_1.isBusinessHours)())
            continue;
        if (pausedUsers && pausedUsers.has(userId))
            continue;
        if (state.reengagementSent)
            continue;
        if (state.cartRecovered)
            continue; // Already got an abandoned cart message
        if (!state.lastActivityAt)
            continue;
        const hours = (0, date_fns_1.differenceInHours)(now, state.lastActivityAt);
        if (hours >= COLD_LEAD_THRESHOLD_HOURS) {
            logger_1.default.info(`[SCHEDULER] Cold lead detected: ${userId} inactive for ${hours}h on "${state.step}"`);
            ...
            try {
                ...
                logger_1.default.error(`[SCHEDULER] Failed to send cold lead message to ${userId}:`, e.message);
            }
        }
    }
}
 ...
 */
async function checkAbandonedCarts(sharedState, dependencies) {
    const { userState, pausedUsers } = sharedState;
    const { sendMessageWithDelay, saveState } = dependencies;
    const now = Date.now();
    for (const [userId, state] of Object.entries(userState)) {
        ...
        if (state.cartRecovered)
            continue;
        if (state.reengagementSent)
            continue; // Already got a cold lead message
        const lastActivity = state.lastActivityAt || state.stepEnteredAt;
        if (!lastActivity)
            continue;
        const hours = (0, date_fns_1.differenceInHours)(now, lastActivity);
        if (hours > ABANDONED_CART_MIN_HOURS && hours < ABANDONED_CART_MAX_HOURS) {
            logger_1.default.info(`[SCHEDULER] Abandoned cart detected: ${userId} inactive for ${hours}h on "${state.step}"`);
            ...
            try {
                ...
                logger_1.default.error(`[SCHEDULER] Failed to send abandoned cart message to ${userId}:`, e.message);
            }
        }
    }
}
 ...
 */
async function checkSecondFollowUp(sharedState, dependencies) {
    const { userState, pausedUsers } = sharedState;
    const { sendMessageWithDelay, saveState } = dependencies;
    const now = Date.now();
    for (const [userId, state] of Object.entries(userState)) {
        ...
        if (pausedUsers && pausedUsers.has(userId))
            continue;
        if (state.secondFollowUpSent)
            continue;
        // Only target users who already got a first follow-up
        if (!state.reengagementSent && !state.cartRecovered)
            continue;
        const lastActivity = state.lastActivityAt || state.stepEnteredAt;
        if (!lastActivity)
            continue;
        ...
        if (hours >= 48 && hours < 72) {
            ...
            try {
                ...
                saveState(userId);
                logger_1.default.info(`[SCHEDULER] Second follow-up sent to ${userId} (${hours}h inactive on "${state.step}")`);
            }
            catch (e) {
                logger_1.default.error(`[SCHEDULER] Failed to send second follow-up to ${userId}:`, e.message);
            }
        }
    }
}
...
function cleanupOldUsers(sharedState, dependencies) {
    ...
    for (const [userId, state] of Object.entries(userState)) {
        ...
        if ((0, date_fns_1.differenceInDays)(now, lastActivity) > CLEANUP_THRESHOLD_DAYS) {
            delete userState[userId];
            cleaned++;
        }
    }
    if (cleaned > 0) {
        logger_1.default.info(`[SCHEDULER] Cleaned up ${cleaned} inactive user(s) (>30 days)`);
        saveState();
    }
}
/**
 ...
 */
function cleanStalePausedUsers(sharedState, dependencies) {
    const { userState, pausedUsers } = sharedState;
    const { saveState } = dependencies;
    const now = Date.now();
    ...
    for (const userId of Array.from(pausedUsers)) {
        ...
        if (ACTIVE_STEPS.has(state.step))
            continue;
        const lastActivity = state.lastActivityAt || state.stepEnteredAt;
        if (!lastActivity) {
            // No timestamp at all — treat as stale and clean up
            logger_1.default.info(`[SCHEDULER] Removing stale pause for ${userId} (no activity timestamp, step: ${state.step})`);
            pausedUsers.delete(userId);
            cleaned++;
            continue;
        }
        if ((0, date_fns_1.differenceInDays)(now, lastActivity) > STALE_PAUSE_DAYS) {
            logger_1.default.info(`[SCHEDULER] Removing stale pause for ${userId} (inactive ${(0, date_fns_1.differenceInDays)(now, lastActivity)} days, step: ${state.step})`);
            pausedUsers.delete(userId);
            cleaned++;
        }
    }
    if (cleaned > 0) {
        logger_1.default.info(`[SCHEDULER] ✅ Cleaned ${cleaned} stale paused user(s) (>7 days inactive)`);
        saveState();
    }
}
// ══════════════════════════════════════════════════════════════
...
function startScheduler(sharedState, dependencies) {
    logger_1.default.info(`[SCHEDULER] ⏰ Iniciando cron jobs (timezone: ${TIMEZONE})`);
    ...
    node_cron_1.default.schedule('*/3 9-23 * * *', () => {
        autoApproveOrders(sharedState, dependencies);
    }, { timezone: TIMEZONE });
    logger_1.default.info('[SCHEDULER] ✅ autoApproveOrders → cada 3 min (9-23h ARG)');
    // ── COLD LEADS: a las 10am y 6pm Argentina ──
    // Horas óptimas para re-engagement (mañana y tarde).
    node_cron_1.default.schedule('0 10,18 * * *', () => {
        checkColdLeads(sharedState, dependencies);
    }, { timezone: TIMEZONE });
    logger_1.default.info('[SCHEDULER] ✅ checkColdLeads → 10:00 y 18:00 ARG');
    ...
    node_cron_1.default.schedule('0 10-21 * * *', () => {
        checkAbandonedCarts(sharedState, dependencies);
    }, { timezone: TIMEZONE });
    logger_1.default.info('[SCHEDULER] ✅ checkAbandonedCarts → cada hora de 10 a 21 ARG');
    ...
    logger_1.default.info('[SCHEDULER] ✅ checkSecondFollowUp → 14:00 ARG (diario)');
    // ── DAILY STATS SNAPSHOT: a las 23:55 Argentina ──
    // Guarda el total de chats en BD antes de perderlos por rotación.
    node_cron_1.default.schedule('55 23 * * *', () => {
        snapshotDailyStats(sharedState);
    }, { timezone: TIMEZONE });
    logger_1.default.info('[SCHEDULER] ✅ snapshotDailyStats → 23:55 ARG (diario)');
    ...
    logger_1.default.info('[SCHEDULER] ✅ cleanupOldUsers → 04:00 ARG (diario)');
    ...
    logger_1.default.info('[SCHEDULER] ✅ cleanStalePausedUsers → 05:00 ARG (diario)');
    ...
    node_cron_1.default.schedule('0 8 * * *', async () => {
        logger_1.default.info('[SCHEDULER] 🔄 Ejecutando reinicio preventivo diario (Anti-Memory Leak)...');
    ...
    logger_1.default.info('[SCHEDULER] ✅ Reinicio Preventivo Diario → 08:00 ARG (diario)');
    ...
    setTimeout(() => {
        ...
        if (argHour >= 9 && argHour < 23) {
            ...
            logger_1.default.info(`[SCHEDULER] Skipping boot-time autoApproveOrders (hour ${argHour} ARG, outside 9-23h)`);
        }
    }, 10000);
 ...
 */
async function snapshotDailyStats(sharedState) {
    try {
        const { prisma } = require('../../db');
        const INSTANCE_ID = process.env.INSTANCE_ID || 'default';
        // Use Argentina timezone so the date is correct regardless of server location
        const argNow = (0, date_fns_tz_1.toZonedTime)(new Date(), TIMEZONE);
        ...
        if (sharedState?.userState) {
            ...
            for (const state of Object.values(sharedState.userState)) {
                if (state.step)
                    counts[state.step] = (counts[state.step] || 0) + 1;
            }
        ...
        logger_1.default.info(`[SCHEDULER] Daily Stats Snapshot saved for ${startOfDay.toISOString()}`);
    }
    catch (e) {
        logger_1.default.error('[SCHEDULER] Failed to save daily stats snapshot:', e);
    }
}
