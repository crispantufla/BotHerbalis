...
---
## 🛑 [21 Mar 2026] Fix: Bot omite objeción temporal y entra en loop al pedir datos
**Problema:** En la etapa `waiting_plan_choice`, si el usuario indicaba que elegiría el plan de 60 días pero debía esperar a cobrar (ej: "espero al 27 pero elijo el de 60"), el texto carecía de signos de exclamación o palabras obvias de duda en el regex interceptor, por lo que el bot macheaba el "60", guardaba el plan y avanzaba a pedir la dirección ignorando la espera. Peor aún, si la IA lograba procesarlo y devolvía flag de `POSTDATADO`, la lógica del framework avanzaba a `waiting_data` PERO omitía guardar el plan en memoria. Al ingresar a `waiting_data` sin plan, el bot entraba en falla: ante cualquier pregunta (como "¿cuánto tarda en llegar?"), la precaución de `waiting_data` la rebotaba al paso anterior reenviando el mensaje original del precio.
**Solución:** Se reparó en `src/flows/steps/stepWaitingPlanChoice.ts`. 1) Se añadieron verbos temporales e interjecciones ("esperar", "cobrar", "sueldo", etc.) a la constante `hasQuestionText` asegurando que toda duda o objeción pase por el embudo de IA sin importar si incluye el número del plan. 2) Se refactorizó la extracción donde, si la IA detecta fecha de cobro (`POSTDATADO`), pero el cliente ya declaró explícitamente el plan ("el de 60"), el sistema captura automáticamente ambas cosas (plan y fecha futura) y las guarda aliviando a `waiting_data` en el futuro.
