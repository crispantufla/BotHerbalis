...
## Botón "Solo registrar" (reporte de Horacio, 24-jul tarde) — RESUELTO
Horacio aclaró: el modal SÍ abre, solo tarda. NO es bug. La demora es la extracción IA del preview (rescata dirección/producto del historial). Aggravada hoy por OpenAI 429: parseAddress reintenta 3× con backoff antes de caer a Claude. Con saldo OpenAI cargado, abre rápido. Usuario OK con la demora actual. Comportamiento intencional: "Solo registrar" abre siempre el modal de verificación primero (ya no registra directo).

## Agente PC de Horacio: warning punycode + relaunch (24-jul tarde)
Síntoma: al abrir el agente aparece "the punycode module is deprecated" y la ventana se cierra/tarda en reabrir.
- **Warning punycode**: DEP0040 de Node por dependencia transitiva (whatsapp-web.js/ws). Cosmético, no afecta nada, pero asusta. FIX: `process.noDeprecation = true` al tope de agent.js (commit — se propaga por /agent-dist en el próximo arranque del agente).
- **Ventana cierra/reabre**: es el ciclo normal de auto-update (exit 99 → run.bat espera 5s → relanza → Chromium cold-start ~10-15s). Se vio más hoy por los múltiples deploys. Tras aplicar el update del agente queda estable; los deploys de backend NO tocan los 6 archivos del agente (agent.js/sidebar.js/updater.js/package*.json/run.bat) así que no lo reinician.
- El update se aplica cuando Horacio reinicie el agente, o vía POST /api/agent/update?force=1 (relanza en el acto — no forzado sin pedir, interrumpe su venta ~20s).
