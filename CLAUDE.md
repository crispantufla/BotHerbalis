...
- **Eco de lo que manda el bot**: todo envío vuelve como `message_create` propio, igual que lo que el vendedor escribe a mano. Se distinguen con `trackBotSends` (`messageHandler.ts`), nunca con una espera fija: en remoto el eco llega ANTES que el ack con el id. El primer deploy del arreglo de `@lid` esperaba 100 ms y se revirtió a los 5 minutos porque el saludo del bot pausó el chat.
