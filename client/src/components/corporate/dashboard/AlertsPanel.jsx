...
const AlertsPanel = ({ alerts, onCommand, onQuickAction }) => {
    ...
    return (
        <div className="lg:col-span-2 space-y-6">
            {/* Header V2 */}
            <div className="flex items-center justify-between bg-white dark:bg-slate-800/40 backdrop-blur-md p-4 sm:p-5 rounded-3xl border border-slate-200/60 dark:border-slate-700/60 shadow-sm">
            ...
            {/* Empty State V2 */}
            {alerts.length === 0 ? (
                <div className="relative overflow-hidden rounded-3xl border border-slate-200/60 dark:border-slate-700/60 bg-white dark:bg-slate-800/40 backdrop-blur-md p-8 sm:p-12 text-center shadow-sm">
                ...
                <div className="space-y-4">
                    {alerts.map((alert, index) => {
                        ...
                        return (
                            <div key={alert.id} className="group overflow-hidden rounded-3xl border border-slate-200/60 dark:border-slate-700/80 bg-white dark:bg-slate-800/60 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300">
                                {/* Header del Alerta */}
                                <div className="p-3 sm:p-6 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors" onClick={() => toggleExpand(alert.id)}>
                                ...
                                {/* Contenido Expandido */}
                                {isExpanded && (
                                    <div className="border-t border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 p-3 sm:p-6">
                                        {isOrderApproval && (
                                            <div className="mb-6 bg-white dark:bg-slate-900/50 rounded-2xl p-4 sm:p-5 border border-slate-100 dark:border-slate-800 shadow-sm">
