    ...
    return (
        <div className={`h-full flex flex-col ${isDark ? 'bg-slate-900' : 'bg-slate-50'} overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600`}>
            <div className="p-4 sm:p-6 lg:p-8 flex-1 flex flex-col min-h-0">
                ...
                {/* --- SECCIÓN B2: RENDIMIENTO POR ANUNCIO --- */}
                {data.adPerformance && data.adPerformance.length > 0 && (
                    <>
                        <h2 className={`text-sm font-bold uppercase tracking-widest mb-4 mt-8 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Rendimiento por Anuncio</h2>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                            {data.adPerformance.map((ad, idx) => {
                                const AD_COLORS = ['#3b82f6', '#8b5cf6', '#f59e0b', '#ec4899', '#10b981'];
                                const color = AD_COLORS[idx % AD_COLORS.length];
                                const STEP_LABELS = {
                                    greeting: 'Saludo',
                                    waiting_weight: 'Peso',
                                    waiting_preference: 'Preferencia',
                                    waiting_plan_choice: 'Plan',
                                    waiting_ok: 'Confirmación',
                                    waiting_data: 'Datos envío',
                                    waiting_final_confirmation: 'Confirmación final',
                                    waiting_admin_ok: 'Aprobación admin',
                                    completed: 'Completado'
                                };
                                return (
                                    <div key={ad.source} className={`p-5 sm:p-6 lg:p-7 rounded-3xl border shadow-sm overflow-hidden ${isDark ? 'bg-slate-800/40 border-slate-700/50' : 'bg-white border-slate-200'}`}>
                                        <div className="mb-5">
                                            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                                <Megaphone size={20} style={{ color }} />
                                                {ad.source === 'orgánico' ? 'Tráfico Orgánico' : ad.source.replace('anuncio_', 'Anuncio ')}
                                            </h3>
                                            <div className="flex gap-4 mt-2">
                                                <span className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{ad.total} chats</span>
                                                <span className={`text-sm font-bold`} style={{ color }}>{ad.orders} ventas</span>
                                                <span className={`text-sm font-bold`} style={{ color }}>{ad.conversionRate}% conv.</span>
                                                <span className={`text-sm font-medium ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>${ad.revenue?.toLocaleString('es-AR')}</span>
                                            </div>
                                        </div>

                                        {/* Funnel bars */}
                                        <div className="space-y-2">
                                            {ad.funnel.filter(f => f.count > 0).map((step) => {
                                                const maxCount = ad.funnel[0]?.count || 1;
                                                const barWidth = Math.max((step.count / maxCount) * 100, 4);
                                                return (
                                                    <div key={step.step} className="flex items-center gap-3">
                                                        <span className={`text-xs font-medium w-28 text-right truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                                                            {STEP_LABELS[step.step] || step.step}
                                                        </span>
                                                        <div className={`flex-1 h-5 rounded-full overflow-hidden ${isDark ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                                                            <div
                                                                className="h-full rounded-full transition-all duration-500 flex items-center justify-end pr-2"
                                                                style={{ width: `${barWidth}%`, backgroundColor: color, opacity: 0.8 }}
                                                            >
                                                                <span className="text-[10px] font-bold text-white drop-shadow">{step.count}</span>
                                                            </div>
                                                        </div>
                                                        <span className={`text-xs font-mono w-12 text-right ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{step.rate}%</span>
                                                    </div>
                                                );
                                            })}
                                        </div>
